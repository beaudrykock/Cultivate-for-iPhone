...
- (void)viewDidLoad {
	...
	if(!deviceIsTablet) {
        ...
        {
            height = 320; // additional 20.0 pt for status bar
            width = 480;
        }
        else
            // when running in simulator
        {
            width = 320;
            height = 480;
        }
        
        subviewLoading = [[UIActivityIndicatorView alloc] initWithFrame: CGRectMake((width/2)-15, (height/2)-15, 30, 30)];
        subviewLoading.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray;
        subviewLoading.autoresizingMask = (UIViewAutoresizingFlexibleLeftMargin |
                                           UIViewAutoresizingFlexibleRightMargin |
                                           UIViewAutoresizingFlexibleTopMargin |
                                           UIViewAutoresizingFlexibleBottomMargin);
        
        [rWebView addSubview:subviewLoading];
