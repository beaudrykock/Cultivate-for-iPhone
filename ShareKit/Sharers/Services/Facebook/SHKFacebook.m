...
+ (void)logout {
	Facebook *fb = [[[Facebook alloc] initWithAppId:SHKFacebookAppID andDelegate:nil] autorelease];
...
-(NSString*)getAuthorFromText:(NSString*)text
{
    NSScanner *scanner = [NSScanner scannerWithString:text];
    NSString *temp = nil;
    [scanner scanUpToString:@"By" intoString:nil];
    int loc = [scanner scanLocation];
    
    temp = [text substringFromIndex:loc];
...
- (BOOL)send {
	if (item.shareType == SHKShareTypeURL) {
        item.text = [NSString stringWithFormat:@"%@%@%@", [self getAuthorFromText:item.text], @" | ", @"High Country News"];
